...
var (
	...
	targetdatacenters = strings.Split(os.Getenv("DATACENTERS"), ",") // 选择的数据中心, e.g.
	...
	for _, item := range result {
		if item["planCode"] == plancode {
			...
			for _, dcInfo := range datacenters {
				...
				if len(targetdatacenters) > 0 {
					for _, targetdc := range targetdatacenters {
						if datacenter == targetdc {
